import random
import time
from lab3.utils import memory_usage_process, write_file, open_file, print_input_output
...
def partition_upd(arr, left_index, right_index):
    x = arr[left_index]
    m_left = left_index  # индекс для элементов < x
    m_right = right_index  # индекс для элементов > x
    i = left_index  # индекс для текущего элемента
